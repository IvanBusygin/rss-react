...
describe('ArticleCardDetails component', () => {
  it('renders ArticleCardDetails information', () => {
    render(<ArticleCardDetails article={article} />);
    ...
    expect(
      screen.getByText(`Published at: ${article.publishedAt.slice(0, -10)}`),
    ).toBeInTheDocument();
    expect(screen.getByText(article.title)).toBeInTheDocument();
    expect(screen.getByText(`Source: ${article.source.name}`)).toBeInTheDocument();
  });
});
